...
switch ($_POST['accion']) {
	case 'POST_REG_PERSONA':
		$arr_datos[0] = array($_POST['id_tipo_doc_pac'], $_POST['nro_doc_pac'], trim($_POST['nombres_pac']), trim($_POST['primer_ape_pac']), trim($_POST['segundo_ape_pac']), trim($_POST['id_sexo_pac']), $_POST['fec_nac_pac'], trim($_POST['id_pais_pac']), trim($_POST['id_etnia_pac']), trim($_POST['tel_fij_pac']), trim($_POST['tel_mov_pac']), trim($_POST['email_pac']), trim($_POST['id_ubigeo_pac']), trim($_POST['direccion_pac']), trim($_POST['ref_direccion_pac']), trim($_POST['nro_hc_pac']));
  ...
  if ($nr > 0) {
    ...
    if ($_POST['txtIdTipDoc'] == "1"){
      ...
      if ($nr > 0) {
        ...
        echo json_encode($datos);
      } else {//Fin si encontro en el padron
			...
			//Lo del minsa lo desabilite porque bloquearon la IP
		if ($dataminsa == 0) {
