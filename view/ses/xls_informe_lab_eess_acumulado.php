...
$file_path = "plantilla_xls_cnt_eess_acumulado.xlsx";
...
//print_r($anio.$mes_desde.$mes_hasta.$id_dependencia); exit();

//  ------------------- CANTIDAD ATENCIONES
$rsAte = $ses->get_CntDetalleInformeSESLabAtencion('EESSACU', $anio, $mes_desde, $mes_hasta, $id_dependencia);
//print_r($rsAte); exit();
if(count($rsAte) <> 0){
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $max, $rsAte[0]['cnt_sis']);
	$column++;
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $max, $rsAte[0]['cnt_pagante']);
	$column++;
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $max, $rsAte[0]['cnt_estrategia']);
	$column++;
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $max, $rsAte[0]['cnt_exonerado']);
	$column++;
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $max, $rsAte[0]['cnt_total']);
	$column++;
} else {
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $max, 0);
	$column++;
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $max, 0);
	$column++;
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $max, 0);
	$column++;
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $max, 0);
	$column++;
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $max, 0);
	$column++;		
}
$objPHPExcel->getActiveSheet()->setSharedStyle($estiloborde, "C" . $max . ":G" . $max);
...
foreach ($rsTP as $rowTP) {
	...
	$rsTPr = $ses->get_CntDetalleInformeSESLabDetTipoProducto('EESSACU', $anio, $mes_desde, $mes_hasta, $id_dependencia, $rowTP['id']);
	//print_r($rsTPr); exit();
	if(count($rsTPr) <> 0){
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $max, $rsTPr[0]['cnt_sis']);
		$column++;
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $max, $rsTPr[0]['cnt_pagante']);
		$column++;
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $max, $rsTPr[0]['cnt_estrategia']);
		$column++;
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $max, $rsTPr[0]['cnt_exonerado']);
		$column++;
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $max, $rsTPr[0]['cnt_total']);
		$column++;
	} else {
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $max, 0);
		$column++;
	...
	$objPHPExcel->getActiveSheet()->setSharedStyle($estiloNumero, "B" . $max . ":G" . $max);
	...
	if (count($rsPr) > 0) {
	  ...
	  foreach ($rsPr as $row) {
		...
		$rsExa = $ses->get_CntDetalleInformeSESLabTotLabExamenXProd('EESSACU', $anio, $mes_desde, $mes_hasta, $id_dependencia, $row['id_producto']);
		if(count($rsExa) <> 0){
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $max, $rsExa[0]['cnt_sis']);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $max, $rsExa[0]['cnt_pagante']);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $max, $rsExa[0]['cnt_estrategia']);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $max, $rsExa[0]['cnt_exonerado']);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $max, $rsExa[0]['cnt_total']);
		} else {
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $max, 0);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $max, 0);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $max, 0);
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue("C12", "=C" . $maxHema . " + C" . $maxBio . " + C" . $maxImn . " + C" . $maxMic . " + C" . $maxPaq);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue("D12", "=D" . $maxHema . " + D" . $maxBio . " + D" . $maxImn . " + D" . $maxMic . " + D" . $maxPaq);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue("E12", "=E" . $maxHema . " + E" . $maxBio . " + E" . $maxImn . " + E" . $maxMic . " + E" . $maxPaq);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue("F12", "=F" . $maxHema . " + F" . $maxBio . " + F" . $maxImn . " + F" . $maxMic . " + F" . $maxPaq);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue("G12", "=G" . $maxHema . " + G" . $maxBio . " + G" . $maxImn . " + G" . $maxMic . " + G" . $maxPaq);


$objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacionTitulotprod, "C" . $max . ":G" . $max);
