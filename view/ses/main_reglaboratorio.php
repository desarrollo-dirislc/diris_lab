...
$input = & $_POST;
$reg_anio = $input['txt_reg_anio'];
$reg_mes = $input['txt_reg_mes'];
$reg_id_dependencia = $input['txt_id_reg_dependencia'];
...
<div class="container-fluid">
  <div class="panel-prime">
    <div class="panel-heading">
      <h3 class="panel-title"><strong>REGISTRO DE INFORME MENSUAL DE EXAMENES DE LABORATORIO</strong></h3>
    </div>
					...
					<div class="form-group">
						...
						<?php
							...
							for ($i = $year_init; $i <= $year_curent; $i++) {
								echo "<option value='".$i."'"; if($reg_anio == $i){ echo " selected";}  echo ">".$i."</option>";
							}
						?>
						</select>
					</div>
					...
					<div class="form-group">
						...
						<?php
							...
							for ($i = 1; $i <= count($meses_arr); $i++) {
								echo "<option value='".$i."'"; if($reg_mes == $i){ echo " selected";}  echo ">" . $meses_arr[$i - 1] . "</option>";
							}
						?>
						</select>
					</div>
					...
					<div class="form-group">
						...
						<select name="txt_id_dependencia" id="txt_id_dependencia" class="form-control" disabled>
						  ...
						  foreach ($rsD as $row) {
							...
							if ($row['id_dependencia'] == $reg_id_dependencia) echo " selected";
									...
									<div class="tab-content">
										...
										<table class="table table-bordered m-auto">
											...
											<?php
												...
												$tot_estrategia = 0;
												$tot_exonerado = 0;
												$param[0]['anio'] = $reg_anio;
												$param[0]['mes'] = $reg_mes;
												$param[0]['idDepAten'] = $reg_id_dependencia;
											...
											<?php
												...
												if ($nr > 0) {
												  foreach ($rs as $row) {
														...
														$hem_aus = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 1, 1,$row['id_producto'],$row['es_toma_muestra']);
														$hem_pag = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 2, 1,$row['id_producto'],$row['es_toma_muestra']);
														$hem_esa = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 3, 1,$row['id_producto'],$row['es_toma_muestra']);
														$hem_exo = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 4, 1,$row['id_producto'],$row['es_toma_muestra']);
											...
											<?php
												...
												if ($nrbio > 0) {
												  foreach ($rsbio as $row) {
													...
													$bio_aus = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 1, 2,$row['id_producto'],$row['es_toma_muestra']);
													$bio_pag = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 2, 2,$row['id_producto'],$row['es_toma_muestra']);
													$bio_esa = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 3, 2,$row['id_producto'],$row['es_toma_muestra']);
													$bio_exo = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 4, 2,$row['id_producto'],$row['es_toma_muestra']);
											...
											<?php
												...
												if ($nrinm > 0) {
												  foreach ($rsinm as $row) {
													...
													$inm_aus = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 1, 3,$row['id_producto'],$row['es_toma_muestra']);
													$inm_pag = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 2, 3,$row['id_producto'],$row['es_toma_muestra']);
													$inm_esa = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 3, 3,$row['id_producto'],$row['es_toma_muestra']);
													$inm_exo = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 4, 3,$row['id_producto'],$row['es_toma_muestra']);
											...
											<?php
												...
												if ($nrmic > 0) {
												  foreach ($rsmic as $row) {
													...
													$mic_aus = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 1, 4,$row['id_producto'],$row['es_toma_muestra']);
													$mic_pag = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 2, 4,$row['id_producto'],$row['es_toma_muestra']);
													$mic_esa = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 3, 4,$row['id_producto'],$row['es_toma_muestra']);
													$mic_exo = $at->get_repIndicadorProduccionPorAnioMes($reg_anio, $reg_mes, $reg_id_dependencia, 4, 4,$row['id_producto'],$row['es_toma_muestra']);
							...
							<table class="table table-bordered m-auto">
								...
								<?php
									...
									<tr class="<?php echo $trBKColor; ?>">
										<td><?php echo $item_bk . "- " . $row['nombre'];?></td>
										<td>
											...
											<input type="number" name="txt_bk_aten[]" id="txt_bk_aten_<?php echo $row['id'];?>" class="form-control" maxlength="4" onfocus="this.select()" onkeypress="return NumCheck(event);" onkeyup="totales_bk('<?php echo $row['id'];?>');" value="0">
										</td>
										<td><input type="number" name="txt_bk_posi1[]" id="txt_bk_posi1_<?php echo $row['id'];?>" class="form-control" maxlength="4" onfocus="this.select()" onkeypress="return NumCheck(event);" onkeyup="totales_bk('<?php echo $row['id'];?>');" value="0"></td>
										<td><input type="number" name="txt_bk_posi2[]" id="txt_bk_posi2_<?php echo $row['id'];?>" class="form-control" maxlength="4" onfocus="this.select()" onkeypress="return NumCheck(event);" onkeyup="totales_bk('<?php echo $row['id'];?>');" value="0"></td>
										<td><input type="number" name="txt_bk_posi3[]" id="txt_bk_posi3_<?php echo $row['id'];?>" class="form-control" maxlength="4" onfocus="this.select()" onkeypress="return NumCheck(event);" onkeyup="totales_bk('<?php echo $row['id'];?>');" value="0"></td>
										<td><input type="number" name="txt_bk_pau[]" id="txt_bk_pau_<?php echo $row['id'];?>" class="form-control" maxlength="4" onfocus="this.select()" onkeypress="return NumCheck(event);" onkeyup="totales_bk('<?php echo $row['id'];?>');" value="0"></td>
										<td><input type="number" name="txt_bk_tot" id="txt_bk_tot_<?php echo $row['id'];?>" class="form-control" maxlength="5" value="0" disabled=""></td>
								<?php
									}
								?>
								</body>
...
function save_atencion(opt) {
  ...
  if(opt == "LAB"){
	  ...
	  if(ing == ""){
		msg += "LABORATORIO: Ingrese almenos la cantidad de un examen.<br/>";
		sw = false;
	  }
  } else {
	  /*var AnameInput = $('#frmAreaBac').serializeArray();
	  var ing = "";
	  len = AnameInput.length;
	  for (i=0; i<len; i++) {
		nameInput = AnameInput[i]['name'];
		var arrayCadenas = nameInput.split('_');
		if(arrayCadenas.length == 3){
		  if($("input[name=" + nameInput + "]").val().trim() != ""){
			ing = "1";
			break;
		  }
		}
	  ...
	  if(ing == ""){
		msg += "BASILOSCOPIA: ingrese la cantidad de almenos un diagnóstico.<br/>";
		sw = false;
	  }*/
      ...
      if (result == true) {
		if(opt == "LAB"){
			$.ajax( {
			  ...
			  + "&id_informe=0&id_dependencia=" + $('#txt_id_dependencia').val() + "&anio=" + $('#txt_anio').val() + "&mes=" + $('#txt_mes').val()
