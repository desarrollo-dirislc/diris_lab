...
class PDF extends FPDF
{
  ...
  function Header()
  {
    ...
    $this->SetFont('Arial','',7);
    $this->Cell(22,4,utf8_decode(': ' . $rsA[0]['edad_anio'] . ' AÑOS'),0,0,'');
    $this->SetFont('Arial','B',7);
    ...
    $this->Cell(22,4, utf8_decode(': ' . $nroAtencion),0,0,'');
    $this->SetFont('Arial','B',7);
	...
	if($rsA[0]['nombre_medico'] == ""){
		...
		$this->Cell(18,4,utf8_decode(': ' . $rsA[0]['abrev_plan']),0,0,'');
	}
...
$rsA = $pn->get_datosAreaPoridAtencion($idAtencion);
foreach ($rsA as $rowA) {
  $pdf->AddPage();
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(0,3,'',0,1,'');
    $pdf->Cell(0,5,utf8_decode($rowA['abrev_tipo_producto']),1,1,'C');

	$rsP = $pn->get_datosProductoPorIdAtencionAndIdArea($idAtencion,$rowA['id']);
	foreach ($rsP as $rowP) {
	  ...
	  if(strlen($rowP['nom_producto'])<34){
		  ...
		  foreach ($rsG as $rowG) {
			...
			foreach ($rsC as $rowC) {
					...
					//print_r($rowC['id'] ." || " . $rowC['id_tipo_val_ref'] ." || " . $resulVRef ." || " . $edadAnio ." || " . $edadMes ." || " . $edadDia ." || " . $nomSexo);
					//exit();
