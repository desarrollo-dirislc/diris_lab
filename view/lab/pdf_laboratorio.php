...
$rsA = $a->get_listaAreaPorIdAtencion($idAtencion, $idArea);
foreach ($rsA as $rowA) {
  $pdf->AddPage();
  if($rowA['visible'] == "1"){
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(0,3,'',0,1,'');
    $pdf->Cell(0,5,utf8_decode($rowA['area']),1,1,'C');
  ...
  foreach ($rsP as $rowP) {
	...
	foreach ($rsG as $rowG) {
	  ...
	  $pdf->Cell(0,4,utf8_decode($rowG['grupo']),1,1,'');
	}*/
	$pdf->Ln(2);
	...
	foreach ($rsC as $rowC) {
	  if($rowA['visible'] == "1"){
		if($rowC['tipo_ingresosol'] == "1"){
		  ...
		  $pdf->MultiCell(30,4,utf8_decode($totVal),0,'C','');//Valor referencia
